  ...
  <div class="flex min-h-screen">
    ...
    <main class="flex-1 p-8">
      <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-2xl font-heading font-bold">Nouveau produit</h1>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="max-w-2xl space-y-6">
        ...
        <BaseButton type="submit" :loading="submitting">Creer le produit</BaseButton>
      </form>
    </main>
  </div>
</template>
...
onMounted(async () => {
  try {
    const [cats, mrqs] = await Promise.all([
      api.get('/admin/categories'),
      api.get('/admin/marques'),
    ])
    categories.value = cats.data.categories
    marques.value = mrqs.data.marques
  } catch {
  }
})
...
async function handleSubmit() {
  ...
  try {
    ...
    await api.post('/admin/produits', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
    success('Produit cree avec succes')
    router.push('/admin/produits')
  } catch (e) {
    error(e.response?.data?.message || 'Erreur lors de la creation')
  } finally {
